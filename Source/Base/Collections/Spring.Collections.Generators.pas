...
unit Spring.Collections.Generators;
...
type
  TGenerator = class(TCoroutine, IEnumerator)
  strict private
    fCurrent: TValue;
  protected
  ...
  TGenerator<TResult> = class(TEnumerableBase<TResult>, IInterface, IEnumerable<TResult>)
  private
    fAction: Action;
    type
  ...
  TGenerator<T1, TResult> = class(TGenerator<TResult>,
  ...
  TGenerator<T1, T2, TResult> = class(TGenerator<TResult>,
  ...
  TGenerator<T1, T2, T3, TResult> = class(TGenerator<TResult>,
  ...
  TGenerator<T1, T2, T3, T4, TResult> = class(TGenerator<TResult>,
...
constructor TGenerator.Create(const action: Action);
begin
  inherited Create(action);
  IsLooping := False;
end;

function TGenerator.GetCurrent: TValue;
begin
  Result := fCurrent;
end;

procedure TGenerator.Yield(const value: TValue);
begin
  fCurrent := value;
  inherited Yield;
end;
...
{$REGION 'TGenerator<TResult>'}

constructor TGenerator<TResult>.Create(const action: Action);
begin
  inherited Create;
  fAction := action;
end;

function TGenerator<TResult>.GetEnumerator: IEnumerator<TResult>;
var
...
{$REGION 'TGenerator<TResult>.TEnumerator'}

function TGenerator<TResult>.TEnumerator.GetCurrent: TValue;
begin
...
{$REGION 'TGenerator<T1, TResult>'}

constructor TGenerator<T1, TResult>.Create(const action: Action<T1>);
begin
  inherited Create(
    procedure
    begin
...
function TGenerator<T1, TResult>.Bind(const arg1: T1): IEnumerable<TResult>;
begin
  fArg1 := arg1;
  Result := Self;
end;
...
{$REGION 'TGenerator<T1, T2, TResult>'}

constructor TGenerator<T1, T2, TResult>.Create(const action: Action<T1, T2>);
begin
  inherited Create(
    procedure
    begin
...
function TGenerator<T1, T2, TResult>.Bind(const arg1: T1;
...
{$REGION 'TGenerator<T1, T2, T3, TResult>'}

constructor TGenerator<T1, T2, T3, TResult>.Create(
...
function TGenerator<T1, T2, T3, TResult>.Bind(const arg1: T1; const arg2: T2;
...
{$REGION 'TGenerator<T1, T2, T3, T4, TResult>'}

constructor TGenerator<T1, T2, T3, T4, TResult>.Create(
...
function TGenerator<T1, T2, T3, T4, TResult>.Bind(const arg1: T1;
