...
const
  FiberDataOffset = {$IFDEF CPUX64}$20{$ELSE}$10{$ENDIF};
asm
{$IFDEF CPUX64}
  mov rax,gs:[abs FiberDataOffset]
{$ELSE}
  mov eax,fs:[FiberDataOffset]
{$ENDIF}
end;
{$ENDIF}
...
begin
  if AtomicExchange(fInvoking, 1) = 0 then
  try
    inherited Invoke;
